...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.Game = {
        ...
        startNewGame() {
            document.querySelectorAll('.enemy, .ofuda, .small-ofuda, .special-ofuda, .bomb, .explosion, .obstacle, .goal, .heal-item, .mp-full-item, .enemy-bullet, .fireball, .damage-zone, .spike-zone, .seed-bullet, .boss-fan-bullet, .sentry-laser-beam, .homing-missile, .laser-bullet, .tesla-bolt').forEach(el => el.remove());
        ...
        createStage() {
            ...
            if (this.stageData.groundEnemies) {
                this.stageData.groundEnemies.forEach(d => {
                    ...
                    if (d.type === 'security-bot') this.add(new SecurityBot(d.x, d.y));
                });
            }
            if (this.stageData.fixedEnemies) {
                this.stageData.fixedEnemies.forEach(d => {
                    if (d.type === 'turret') this.add(new TurretEnemy(d.x, d.y));
                    if (d.type === 'sentry-drone') this.add(new SentryDrone(d.x, d.y));
                    if (d.type === 'tesla-coil') this.add(new TeslaCoil(d.x, d.y));
                });
            }
            ...
            if (this.stageData.bossX) {
                ...
                if (this.stageData.bossType === 'final-boss') {
                    this.boss = new CerberusCore(this.stageData.bossX, bossY);
                } else {
    ...
    // ... (Player, Obstacle, Items, Ofuda, Effects... などのクラスは変更なし) ...
    ...
    class Player extends GameObject {
        ...
        handleAllCollisions() {
            ...
            [...enemies, ...projectiles].forEach(hazard => {
                let collision = false;
                if (hazard instanceof SentryLaserBeam) {
                    collision = hazard.checkCollisionWith(this);
                } else {
                    collision = Game.checkRectCollision(this, hazard);
                }

                if (collision) {
                    if (hazard.isBoss) return;
    ...
    class LaserBullet extends GameObject { constructor(x, y, speed) { super('laser-bullet'); this.type = 'enemy-bullet'; this.width = 24; this.height = 4; this.x = x - this.width / 2; this.y = y - this.height / 2; this.vx = speed; this.damage = 15; } update() { this.x += this.vx; if (this.x < Game.cameraX - 50 || this.x > Game.cameraX + Game.WIDTH + 50) { this.destroy(); } } }
    class SecurityBot extends Enemy { constructor(x, platformY) { super('security-bot-enemy', 120, { scoreValue: 50, gaugeValue: 25, damage: 20 }); this.width = 64; this.height = 48; this.x = x; this.y = platformY - this.height; this.vx = (Math.random() < 0.5 ? 1 : -1) * 3.5; this.vy = 0; this.prevX = this.x; this.prevY = this.y; this.shootCooldown = 1500; this.shootTimer = this.shootCooldown * Math.random(); } update(deltaTime) { if (this.isDestroyed) return; this.shootTimer -= deltaTime; const player = Game.player; if (this.shootTimer <= 0 && player && !player.isDestroyed) { const distance = Math.abs(player.x - this.x); if (distance < 600 && Math.abs(player.y - this.y) < 50) { const direction = (player.x > this.x) ? 1 : -1; Game.add(new LaserBullet(this.x + this.width / 2, this.y + 12, direction * 10)); this.shootTimer = this.shootCooldown; this.vx = 0; setTimeout(() => { this.vx = direction * 3.5; }, 500); } } this.x += this.vx; this.vy += Game.GRAVITY; this.y += this.vy; const platforms = Game.gameObjects.filter(obj => obj.isPlatform); let onGround = false; platforms.forEach(p => { if (!Game.checkRectCollision(this, p)) return; if (this.vy >= 0 && this.prevY + this.height <= p.y) { this.y = p.y - this.height; this.vy = 0; onGround = true; } else if (this.y + this.height > p.y && this.prevY < p.y + p.height) { if (this.vx > 0 && this.prevX + this.width <= p.x) { this.x = p.x - this.width; this.vx *= -1; } else if (this.vx < 0 && this.prevX >= p.x + p.width) { this.x = p.x + p.width; this.vx *= -1; } } }); if (onGround) { const edgeCheckX = this.vx > 0 ? this.x + this.width : this.x; const groundCheckPos = { x: edgeCheckX, y: this.y + this.height + 1, width: 1, height: 1 }; if (!platforms.some(p => Game.checkRectCollision(groundCheckPos, p))) { this.x -= this.vx; this.vx *= -1; } } this.prevX = this.x; this.prevY = this.y; } }
    class SentryLaserBeam extends GameObject { constructor(owner) { super('sentry-laser-beam'); this.type = 'damage-zone'; this.owner = owner; this.damage = 8; this.damageInterval = 200; this.width = 1; this.height = 10; this.angle = 0; this.active = false; this.element.style.display = 'none'; this.element.style.transformOrigin = 'left center'; } update() { if (this.owner.isDestroyed || !this.active) { if (this.element.style.display !== 'none') this.element.style.display = 'none'; return; } if (this.element.style.display !== 'block') this.element.style.display = 'block'; const player = Game.player; const startX = this.owner.x + this.owner.width / 2; const startY = this.owner.y + this.owner.height / 2; const dx = player.x + player.width / 2 - startX; const dy = player.y + player.height / 2 - startY; this.angle = Math.atan2(dy, dx); const distance = Math.hypot(dx, dy); this.x = startX; this.y = startY - this.height / 2; this.element.style.width = `${distance}px`; this.element.style.transform = `rotate(${this.angle}rad)`; this.width = distance; } checkCollisionWith(player) { if (!this.active) return false; const startX = this.owner.x + this.owner.width / 2; const startY = this.owner.y + this.owner.height / 2; const endX = startX + Math.cos(this.angle) * this.width; const endY = startY + Math.sin(this.angle) * this.width; const playerCx = player.x + player.width / 2; const playerCy = player.y + player.height / 2; const dx = endX - startX; const dy = endY - startY; const lenSq = dx * dx + dy * dy; if (lenSq === 0) return Math.hypot(playerCx - startX, playerCy - startY) < player.width / 2; let t = ((playerCx - startX) * dx + (playerCy - startY) * dy) / lenSq; t = Math.max(0, Math.min(1, t)); const closestX = startX + t * dx; const closestY = startY + t * dy; const distSq = Math.pow(playerCx - closestX, 2) + Math.pow(playerCy - closestY, 2); return distSq < Math.pow(player.width / 2 + this.height / 2, 2); } }
    class SentryDrone extends Enemy { constructor(x, y) { super('sentry-drone-enemy', 150, { scoreValue: 80, gaugeValue: 30, damage: 0 }); this.width = 48; this.height = 48; this.x = x; this.y = y; this.originalX = x; this.originalY = y; this.state = 'patrol'; this.stateTimer = 2000; this.detectionRange = 400; this.laser = new SentryLaserBeam(this); Game.add(this.laser); } update(deltaTime) { if (this.isDestroyed) return; const player = Game.player; const distance = Math.hypot(player.x - this.x, player.y - this.y); this.stateTimer -= deltaTime; switch(this.state) { case 'patrol': this.laser.active = false; this.y = this.originalY + Math.sin(performance.now() / 1000) * 20; if (distance < this.detectionRange) { this.state = 'tracking'; this.stateTimer = 1500; } break; case 'tracking': this.laser.active = false; const targetX = player.x + (player.direction === 'right' ? -100 : 100); const targetY = player.y - 100; this.x += (targetX - this.x) * 0.02; this.y += (targetY - this.y) * 0.02; if (this.stateTimer <= 0) { this.state = 'firing'; this.stateTimer = 3000; } break; case 'firing': this.laser.active = true; if (this.stateTimer <= 0) { this.state = 'patrol'; this.stateTimer = 2000; } break; } } destroy(options){ this.laser.destroy(); super.destroy(options); } }
    
    // ▼▼▼ 難易度調整: TeslaCoil のクールダウン延長 ▼▼▼
    class TeslaCoil extends Enemy { constructor(x, y) { super('tesla-coil-enemy', 1, { showHpBar: false, scoreValue: 0, gaugeValue: 0 }); this.width = 80; this.height = 40; this.x = x; this.y = y; this.isPlatform = true; this.bolt = new GameObject('tesla-bolt'); this.bolt.type = 'damage-zone'; this.bolt.damage = 40; this.bolt.damageInterval = 500; this.bolt.x = this.x + (this.width - 12) / 2; this.bolt.y = this.y + this.height; this.bolt.width = 12; this.bolt.height = 0; Game.add(this.bolt); this.state = 'idle'; this.stateTimer = 4000 + Math.random() * 2000; } update(deltaTime) { this.stateTimer -= deltaTime; switch(this.state) { case 'idle': this.bolt.height = 0; this.bolt.draw(); if(this.stateTimer <= 0) { this.state = 'charging'; this.stateTimer = 1200; this.element.style.filter = "drop-shadow(0 0 5px #fff)"; } break; case 'charging': if(this.stateTimer <= 0) { this.state = 'firing'; this.stateTimer = 500; this.bolt.height = Game.HEIGHT; this.bolt.draw(); } break; case 'firing': if(this.stateTimer <= 0) { this.state = 'idle'; this.stateTimer = 5000; this.element.style.filter = "none"; } break; } } takeDamage(amount) {} destroy(options){ this.bolt.destroy(); super.destroy(options); } }
    
    // ▼▼▼ 難易度調整: HomingMissile の追尾性能を低下 ▼▼▼
    class HomingMissile extends GameObject { constructor(x, y, target) { super('homing-missile'); this.type = 'enemy-bullet'; this.width = 16; this.height = 24; this.x = x; this.y = y; this.target = target; this.speed = 3.5; this.turnSpeed = 0.04; this.vx = 0; this.vy = -this.speed; this.angle = -Math.PI / 2; this.damage = 25; this.life = 8000; } update(deltaTime) { this.life -= deltaTime; if (this.life <= 0) { Game.add(new Explosion(this.x, this.y)); this.destroy(); return; } if (this.target && !this.target.isDestroyed) { const targetAngle = Math.atan2(this.target.y - this.y, this.target.x - this.x); let angleDiff = targetAngle - this.angle; while (angleDiff > Math.PI) angleDiff -= 2 * Math.PI; while (angleDiff < -Math.PI) angleDiff += 2 * Math.PI; this.angle += Math.max(-this.turnSpeed, Math.min(this.turnSpeed, angleDiff)); } this.vx = Math.cos(this.angle) * this.speed; this.vy = Math.sin(this.angle) * this.speed; this.x += this.vx; this.y += this.vy; this.element.style.transform = `rotate(${this.angle + Math.PI / 2}rad)`; } }
    
    // ▼▼▼ 難易度調整: CerberusCore の攻撃頻度を低下 ▼▼▼
    class CerberusCore extends Enemy {
        constructor(x, y) { super('final-boss-enemy', 1, { isBoss: true, scoreValue: 5000, showHpBar: false }); this.width = 240; this.height = 200; this.x = x; this.y = y; this.coreContainer = new GameObject('boss-core-container'); this.coreContainer.x = this.x; this.coreContainer.y = this.y; this.element.appendChild(this.coreContainer.element); this.cores = [ { pos: {x: 104, y: 20}, hp: 2500, maxHp: 2500, active: true, el: null }, { pos: {x: 30, y: 140}, hp: 2500, maxHp: 2500, active: true, el: null }, { pos: {x: 178, y: 140}, hp: 2500, maxHp: 2500, active: true, el: null } ]; this.cores.forEach(core => { core.el = new GameObject('final-boss-core'); core.el.x = core.pos.x; core.el.y = core.pos.y; this.coreContainer.element.appendChild(core.el.element); }); this.activeCoreIndex = 0; this.cores[this.activeCoreIndex].el.element.classList.remove('invincible'); this.attackTimer = 4000; this.phase = 1; }
        update(deltaTime) { if (this.isDestroyed) return; this.coreContainer.x = this.x; this.coreContainer.y = this.y; this.coreContainer.draw(); this.attackTimer -= deltaTime; if (this.attackTimer <= 0) { this.chooseAttack(); } }
        chooseAttack() { const attacks = ['missile', 'laser', 'barrage']; const attack = attacks[Math.floor(Math.random() * attacks.length)]; const player = Game.player; switch(attack) { case 'missile': Game.add(new HomingMissile(this.x + 40, this.y, player)); Game.add(new HomingMissile(this.x + this.width - 40, this.y, player)); this.attackTimer = 3500 / this.phase; break; case 'laser': Game.add(new LaserBullet(this.x, this.y + 100, 15)); Game.add(new LaserBullet(this.x + this.width, this.y + 120, -15)); this.attackTimer = 2500 / this.phase; break; case 'barrage': for(let i = 0; i < 5; i++) { setTimeout(() => { const angle = Math.random() * Math.PI - Math.PI; Game.add(new EnemyBullet(this.x + this.width/2, this.y, angle)); }, i * 150); } this.attackTimer = 4500 / this.phase; break; } }
        takeDamage(amount) { const core = this.cores[this.activeCoreIndex]; if (!core.active) return; core.hp -= amount; Game.playSE('hit'); core.el.element.style.filter = 'brightness(2)'; setTimeout(() => core.el.element.style.filter = '', 100); if (core.hp <= 0) { core.active = false; core.el.element.style.backgroundColor = '#111'; Game.add(new Explosion(this.x + core.pos.x, this.y + core.pos.y)); const activeCores = this.cores.filter(c => c.active); if (activeCores.length === 0) { this.destroy(); } else { this.cores.forEach(c => c.el.element.classList.add('invincible')); this.activeCoreIndex = this.cores.findIndex(c => c.active); this.cores[this.activeCoreIndex].el.element.classList.remove('invincible'); if (activeCores.length === 1 && this.phase === 1) { this.phase = 1.5; } } } } // フェーズ2の攻撃速度上昇を緩和
        destroy() { if(this.isDestroyed) return; this.isDestroyed = true; for(let i = 0; i < 20; i++) { setTimeout(() => { const ex = this.x + Math.random() * this.width; const ey = this.y + Math.random() * this.height; Game.add(new Explosion(ex, ey)); }, i * 150); } setTimeout(() => { Game.add(new Goal(this.x + this.width / 2 - 50, this.y + this.height - 200, 100, 200)); Game.addScore(this.scoreValue); this.element.remove(); }, 3000); }
    }
});
